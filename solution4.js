...
//register a callback that the runtime will call when the operation succeeds or fails.
...
// returns a random number between 0 and1.
...
//checks if the place is reserved or not.
function isThisPlaceAvailable(place) {
	return places.some((p) => p.row === place.row && p.column === place.column);
}
//هنا asyncدايم تجي مع ال await ووظيفتها تشغل البوت او اليوزر الى ان تطلع النتيجه
...
//هنا نحدد كل طرف كم مره المفروض يلعب
